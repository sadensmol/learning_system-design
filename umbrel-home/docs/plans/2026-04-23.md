...
flowchart LR
  subgraph Device["Umbrel Home (quad-core ARM, ~4 GB RAM)"]
    ...
    Watch[Change watcher<br/>inotify + DB tailers]
    Walker[Filesystem walker<br/>enumerate + stable-read]
...
|---|---|---|---|
| D-1 | Encryption model | Envelope encryption: per-file DEK, wrapped by per-user KEK, wrapped per-device. Master secret never leaves client. See [`pipeline-04-encryption.md`](../pipeline-04-encryption.md). | Gives zero-knowledge while still allowing multi-device and revocation. |
| D-2 | Chunking | Content-defined chunking (FastCDC, ~1 MB avg) for media, Mongo oplog, and SQLite WAL streams; whole-file for tiny files (< 256 KB). See [`pipeline-03-chunking.md`](../pipeline-03-chunking.md) and the standalone [`../../../cdc-guide.md`](../../../cdc-guide.md). | CDC handles insertions well; DB change streams look like growing files, which CDC excels at. |
| D-3 | Dedup scope | Per-user only. No cross-user (convergent) dedup. See [`pipeline-04-encryption.md`](../pipeline-04-encryption.md). | Cross-user dedup leaks plaintext equality — incompatible with zero-knowledge. |
| D-4 | Versioning | Immutable snapshots referencing a Merkle tree of encrypted chunks. Retention as pinned snapshot set; GC removes unreferenced chunks. See [`pipeline-06-storage.md`](../pipeline-06-storage.md). | Time-travel, trash, and retention collapse into one primitive. |
| D-5 | Transport | HTTPS with resumable chunked uploads. See [`pipeline-05-transport.md`](../pipeline-05-transport.md). | Constrained-device friendly, interops with standard infra. |
| D-6 | Local DB backup | **MongoDB:** oplog tailing + periodic base snapshots. **SQLite:** Litestream-style WAL shipping + periodic `VACUUM INTO` base snapshots. Both fenced with a shared blob-ingest watermark for cross-system PIT consistency. See [`databases.md`](../databases.md). | Near-zero RPO for both DB families; each rides the same chunk pipeline; restore = base + change-log replay. |
| D-7 | Multi-device access | Device pairing transfers wrapped KEK over a short-lived, out-of-band channel. Server only ever sees ciphertext KEKs. See [`multi-device.md`](../multi-device.md). | Preserves E2E while allowing phone/web clients. |
...
| D-10 | Per-app isolation | Each Umbrel app is backed up as a distinct *resource group* inside the snapshot: its data directory, its DBs (any mix of Mongo/SQLite), and metadata. Restore is per-app-possible. See [`databases.md`](../databases.md). | Keeps blast radius small when one app misbehaves. Enables partial restore workflows. |
...
- **Change watcher** — detects *when* any source has new data (inotify
  for files, tailable cursors for Mongo, inotify-hint + polling for
  SQLite WAL). See [`pipeline-01-detection.md`](../pipeline-01-detection.md). Routes
  events to:
  - **Filesystem walker** — enumerates files, compares against the
    agent-state DB, streams changed bytes through stable-read into the
    chunker. Used by the watcher (continuous path) and by the one-shot
    coordinator. See [`pipeline-02-file-capture.md`](../pipeline-02-file-capture.md).
  - **Mongo oplog tailer** — one per Mongo instance, follows `oplog.rs`.
  - **SQLite WAL shipper** — one per SQLite file, observes WAL frames
    and periodic `VACUUM INTO` base snapshots.
...
Details in [`databases.md`](../databases.md).
...
Approach (detail in [`databases.md`](../databases.md)):
...
### Pipeline stages (each elaborates one stage of the continuous flow above)

- [`pipeline-01-detection.md`](../pipeline-01-detection.md) — inotify
  for files, Mongo tailable cursors, SQLite WAL polling, cursors,
  single-flight for periodic base jobs.
- [`pipeline-02-file-capture.md`](../pipeline-02-file-capture.md) —
  filesystem walker: enumeration, stable-read, state DB (`files`,
  `walker_runs`), relationship to chunking/dedup.
- [`pipeline-03-chunking.md`](../pipeline-03-chunking.md) — CDC
  algorithm, hash sizing, dedup scope, the skip-upload protocol.
- [`pipeline-04-encryption.md`](../pipeline-04-encryption.md) —
  envelope encryption layout, key wrapping, recovery model, why no
  convergent encryption.
- [`pipeline-05-transport.md`](../pipeline-05-transport.md) —
  resumable chunked upload, back-pressure, device-policy gates.
- [`pipeline-06-storage.md`](../pipeline-06-storage.md) — snapshot
  Merkle tree, retention policy as a pinned-snapshot set, GC protocol.

### Cross-cutting concerns

- [`databases.md`](../databases.md) — unified MongoDB + SQLite
  specialized backup pipeline, fence watermark, per-app restore
  protocol.
- [`multi-device.md`](../multi-device.md) — pairing flow, web client
  constraints, revocation.

### Additional operational flows (beyond the continuous pipeline)

- [`flow-one-shot.md`](../flow-one-shot.md) — user-triggered "back up
  now" and whole-device restore paths; coordinator, fence,
  resumability, failure matrix.

### Standalone reference

- [`../../../cdc-guide.md`](../../../cdc-guide.md) — standalone deep dive
  on CDC: Rabin / Buzhash / FastCDC algorithms, worked byte-level
  example, per-data-type chunking (photos, videos, Mongo oplog,
  SQLite WAL), streaming, resumability, ARM throughput.
